...
ipcMain.handle(
  "save-proxy",
  async (_event, name: string, config: ProxyConfig) => {
    try {
      await saveProxyConfig(name, config);
      return { success: true };
    } catch (error) {
      return {
        success: false,
...
ipcMain.handle("load-proxy", async (_event, name: string) => {
  try {
    const config = await loadProxyConfig(name);
    if (!config) {
...
ipcMain.handle("list-proxies", async () => {
  try {
...
ipcMain.handle("delete-proxy", async (_event, name: string) => {
  try {
