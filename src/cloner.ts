...
export class WebsiteCloner {
  ...
  private async processUrl(url: string, depth: number): Promise<void> {
    ...
    if (!this.config.followExternalLinks && !isSameOrigin(normalizedUrl, this.baseUrl)) {
      return;
    }
    ...
    console.log(`📄 [${this.stats.downloadedPages + 1}] Processing: ${normalizedUrl}`);
    ...
    try {
      ...
      console.error(`❌ Failed to process ${normalizedUrl}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async downloadAsset(url: string, refererUrl: string): Promise<void> {
    ...
    try {
      ...
      this.stats.failedDownloads++;
      console.error(`  ✗ Failed to download ${normalizedUrl}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  private async processCSSFile(filePath: string, cssUrl: string): Promise<void> {
    try {
      ...
      console.error(`  ✗ Failed to process CSS ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  private async inlineSvgSprites(html: string, pageUrl: string): Promise<string> {
  ...
  private printStats(): void {
    ...
    console.log(`✓ Pages downloaded: ${this.stats.downloadedPages}/${this.stats.totalPages}`);
    console.log(`✓ Assets downloaded: ${this.stats.downloadedAssets}/${this.stats.totalAssets}`);
